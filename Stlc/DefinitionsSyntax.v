...
Program Fixpoint weaken_exp_le {n m} (prf : Squash (n <= m)) (e : exp n) : exp m :=
  ...
  | var_b b m => var_b b _
  | var_f x => var_f x
  | abs t => abs (weaken_exp_le _ t)
  | app f t => app (weaken_exp_le prf f) (weaken_exp_le prf t)
  end.

Program Definition weaken_exp {n} (e : exp n) : exp (S n) := weaken_exp_le _ e.


(* Substitute for a free variable. *)
Fixpoint subst_exp_wrt_exp {n} (u:exp n) (y:var) (e:exp n) : exp n :=
  match e with
  | var_b m prf => var_b m prf
  | var_f x => if x == y then u else var_f x
  | abs e1 => abs (subst_exp_wrt_exp (weaken_exp u) y e1)
  | app e1 e2 => app (subst_exp_wrt_exp u y e1) (subst_exp_wrt_exp u y e2)
  end.
...
Program Fixpoint close_exp_wrt_exp {k : nat} (x1 : var) (e1 : exp k) : exp (S k) :=
  ...
  | app e2 e3 => app (close_exp_wrt_exp x1 e2) (close_exp_wrt_exp x1 e3)
  end.
